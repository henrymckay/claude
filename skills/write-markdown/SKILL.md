...
- **Write action sentences imperatively too, not just headings.** Lead an instruction with a verb — "Install the CLI", not "The CLI can be installed". Leave a sentence that states what something *is* declarative — "`CLAUDE.md` holds the project's rules" — the same way reference-section headings stay noun phrases.
- **Fenced code blocks with a language tag** (` ```bash `, ` ```python `), and inline `code` for filenames, commands, flags, identifiers, and tool, package, or library names. Monospace them whether or not they are also linked.
  - **Inside backticks, use the real code spelling, not the brand capitalisation.** Backticks mark a token as code, so write the import or command name: `polars` not `Polars`, `numpy` not `NumPy`, `fastapi` not `FastAPI`. (An *unmonospaced* brand mention in prose or a title may keep its capitalisation — "Polars is fast".)
